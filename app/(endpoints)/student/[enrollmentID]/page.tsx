...
export async function generateMetadata(
  ...
  const studentData: StudentProfileData = await getStudentProfileData(
    params.enrollmentID,
  );
...
export default async function Page({
  ...
  studentData?.subject.forEach((semester) => {
    semester.subjects.forEach((subject) => {
      if (subject.grade) {
        let grade = frequencygrades.find(
          (grade) => grade.grade === subject.grade,
        );
        if (grade) {
          grade.frequency += 1;
        } else {
  ...
  return (
    <>
      ...
      <div className={"w-full px-3 md:px-6 flex-row gap-6"}>
        <div
          className={
            "w-full flex flex-row justify-between text-2xl font-bold scroll-m-20 tracking-tight lg:text-5xlscroll-m-20 pb-2 first:mt-0"
          }
        >
          <h1>Hi, {studentData?.name}</h1>
          <h2>{studentData?.batch}</h2>
        </div>
        <div
          className={
            "grid  md:grid-cols-2 lg:grid-cols-4 justify-between gap-4"
          }
        >
          <Card>
            ...
            <CardContent className="font-semibold p-5">
              {studentData?.sid}
              <CardDescription>SID</CardDescription>
            </CardContent>
          </Card>
          <Card className="border">
            ...
            <CardContent className="font-semibold p-5">
              ...
              <CardDescription>Institute Code</CardDescription>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="rounded-t-2xl border-b border-gray-200 dark:border-gray-800">
            ...
            <CardContent className="font-semibold p-5">
              ...
              <CardDescription>Programme Code</CardDescription>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="rounded-t-2xl border-b border-gray-200 dark:border-gray-800">
        ...
        <ResultSemTabs
          ...
          frequencyGrades={frequencygrades}
        />
      </div>
    </>
  );
