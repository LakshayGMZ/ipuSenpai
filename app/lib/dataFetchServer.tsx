...
export async function getResult(
  ...
  return {
    totalPages: parseInt(res.headers.get("x-total-page-count")!),
    ...(await res.json()),
  };
}
...
export async function getInstitutes(
  progname: string,
): Promise<RanklistQueryFields[]> {
  const res = await fetch(
    getAbsoluteUrl("/institutes/programme=" + encodeURI(progname)),
  );
...
export async function getSpecs(
  ...
  const res = await fetch(
    getAbsoluteUrl(
      `/specializations/programme=${encodeURI(progname)}&institute=${encodeURI(institute)}`,
    ),
  );
...
export async function getShifts(
  ...
  const res = await fetch(
    getAbsoluteUrl(`/institute/shifts/${encodeURI(institute)}`),
  );
...
export async function getBatches(
  ...
  const res = await fetch(
    getAbsoluteUrl(
      `/batches/programme=${encodeURI(progname)}&institute=${encodeURI(institute)}`,
    ),
  );
...
export async function getSemesters(
  ...
  const res = await fetch(
    getAbsoluteUrl(
      `/semesters/programme=${encodeURI(progname)}&institute=${encodeURI(institute)}&batch=${encodeURI(batch)}`,
    ),
  );
...
export async function getSearchByStudentResult(
  options: SearchSelectDataFields,
): Promise<StudentSearchCard[]> {
  const url = new URL(
    getAbsoluteUrl("/student/search/" + options.name.toUpperCase()),
  );
  if (options.institute !== "All Institutes")
    url.searchParams.append("institute", options.institute);
  if (options.programme !== "All Programmes")
    url.searchParams.append("programme", options.programme);
...
export async function getQueriedSubjects(
  subname: string,
): Promise<SubjectSearchData[]> {
  const res = await fetch(getAbsoluteUrl(`/search/subjects/${subname}`));
  if (!res.ok) {
    throw new Error("Error: " + (await res.json())["message"]);
  }
  return await res.json();
